...
// Create booking (public endpoint, not authenticated)
app.post('/api/book', async (req, res) => {
  try {
    ...
    const checkinDate = new Date(checkin), checkoutDate = new Date(checkout), today = new Date();
    ...
    if (typeof children !== 'undefined' && (isNaN(children) || Number(children) < 0))
      return res.status(400).json({ error: 'Number of children must be zero or more' });
    ...
    // Normalize location to lower-case key for price lookup
    const normalizedLocation = location.trim().toLowerCase();
    const bookingPrice = PROPERTY_PRICES[normalizedLocation] || 1200;
